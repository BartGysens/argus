        ...
        <div id="profile-admin" class="clearfix">
            <div id="profile-admin-left">
                <div class="field field-label-inline clearfix">
                    ...
                    <div class="field-items"><?php print smartschool_render('user', $account, 'field_user_sms_naam'); ?>, <?php print smartschool_render('user', $account, 'field_user_sms_voornaam'); ?><?php if (isset($user_class)){ ?> - <a href="<?php print base_path(); ?>klas/<?php print $user_class['title']; ?>"><?php print $user_class['title']; ?></a><?php } ?></div>
                </div>

                <div class="field field-label-inline clearfix">
                    <div class="field-label">Geboortegegevens:&nbsp;</div>
                    <div class="field-items"><?php if (isset($birthday)) { print ($birthday->format('d/m/Y')); } ?>, <?php print smartschool_render('user', $account, 'field_user_sms_geboorteplaats'); ?> - <?php print smartschool_render('user', $account, 'field_user_sms_geboorteland'); ?>, <span style="color: red"><?php print date_diff($birthday,new DateTime('now'))->format('%y jaar'); ?></span></div>
                </div>

                <div class="field field-label-inline clearfix">
                    <div class="field-label">Adres:&nbsp;</div>
                    <div class="field-items"><?php print smartschool_render('user', $account, 'field_user_sms_straat'); ?> <?php print smartschool_render('user', $account, 'field_user_sms_huisnummer'); ?> <?php print smartschool_render('user', $account, 'field_user_sms_busnummer'); ?>, <?php print smartschool_render('user', $account, 'field_user_sms_postcode'); ?> <?php print smartschool_render('user', $account, 'field_user_sms_woonplaats'); ?> - <?php print smartschool_render('user', $account, 'field_user_sms_gezinssituatie'); ?></div>
                </div>
	    ...
	    <div class="container">
		    ...
		    <div>
			    ...
			    <fieldset id="panel-afwezigheden">
		            ...
		            <div class="field field-label-inline clearfix">
		                ...
		                <div class="field-items"><strong><?php print $hotline['absences']['totals']['present']; ?></strong> aanwezig vs. <strong><?php print $hotline['absences']['totals']['absent']; ?></strong> afwezig<?php if ($hotline['absences']['totals']['registered']) { ?> (<strong style="color: red;"><?php print round(100/$hotline['absences']['totals']['registered']*$hotline['absences']['totals']['absent']); ?>%</strong>)<?php } ?></div>
		            </div>
		    ...
		    <div>
				...
				<fieldset id="panel-gedrag">
		            ...
		            <?php if (isset($hotline['behaviour']['first-T-code'])){
				    ...
				    <?php if ($hotline['stage']['total']) { ?>
				        <fieldset id="panel-stages">
				            ...
				            <?php if (count($hotline['stage']['periodgraph'])>1){ ?>
				                ...
				                <div id="stage_chart_periods" style="width: 98%; height: 200px;"></div>
				...
				<fieldset id="panel-studiebegeleiding">
		            ...
		            <?php
		                ...
		                if (array_key_exists(LANGUAGE_NONE, $account->field_user_sms_andere_ls)){
		                	if ($value = smartschool_render('user', $account, 'field_user_sms_andere_ls')){
		            ...
		            <?php
		                $medicalProblems = '';
		                $fieldAsOptions = argus_sms_fieldsAsArrays(variable_get('argus_sms_data_accounts_sync_fieldmap_options'));
		                $medicalProbs = array('Astma','Allergie','Epilepsie','Diabetes','Concentratieproblemen');
		                ...
		                if (array_key_exists(LANGUAGE_NONE, $account->field_user_sms_andere_mp)){
		                	if ($value = smartschool_render('user', $account, 'field_user_sms_andere_mp')){
		            ...
		            if ($medicalProblems){ ?>
		                <div class="field clearfix">
		                    ...
		                    <div class="field-items"><ul><?php print $medicalProblems; ?></ul></div>
		                </div>
		                <hr />
		            <?php } ?>
		                    ...
		                    <div class="field-items"><ul>
		                        ...
		                        if (count($account->field_user_sms_sl_schooljaar)){
		                            ...
		                            if (array_key_exists(LANGUAGE_NONE, $account->field_user_sms_sl_schooljaar)){
			                            if ($account->field_user_sms_sl_schooljaar[LANGUAGE_NONE][0]['safe_value']){
			                                $trajectTitle = '<h5>'.smartschool_render('user', $account, 'field_user_sms_sl_schooljaar').'</h5>';
			                            }
		                            }
		                            ...
		                            if (array_key_exists(LANGUAGE_NONE, $account->field_user_sms_sl_school)){
				                        if ($account->field_user_sms_sl_school[LANGUAGE_NONE][0]['safe_value']){
			                                $traject .= '<li>'.smartschool_render('user', $account, 'field_user_sms_sl_school');
			                                if (array_key_exists(LANGUAGE_NONE, $account->field_user_sms_sl_stad_school)){
				                                if ($account->field_user_sms_sl_stad_school[LANGUAGE_NONE][0]['safe_value']){
				                                    $traject .= ' ('.smartschool_render('user', $account, 'field_user_sms_sl_stad_school').')';
				                                }
			                                }
			                                $traject .= '</li>';
			                            }
		                            ...
		                            if (array_key_exists(LANGUAGE_NONE, $account->field_user_sms_sl_leerjaar_en_st)){
				                        if ($account->field_user_sms_sl_leerjaar_en_st[LANGUAGE_NONE][0]['safe_value']){
			                                $traject .= '<li>'.smartschool_render('user', $account, 'field_user_sms_sl_leerjaar_en_st').'</li>';
			                            }
		                            }
		                            ...
		                            if (array_key_exists(LANGUAGE_NONE, $account->field_user_sms_sl_attest)){
				                        if ($account->field_user_sms_sl_attest[LANGUAGE_NONE][0]['safe_value']){
			                                $traject .= '<li><strong>'.smartschool_render('user', $account, 'field_user_sms_sl_attest').'</strong> behaald</li>';
			                            }
		                            }
		                            ...
		                            if (array_key_exists(LANGUAGE_NONE, $account->field_user_sms_sl_clausulering)){
				                        if ($account->field_user_sms_sl_clausulering[LANGUAGE_NONE][0]['safe_value']){
			                                $traject .= '<li><u>clausule</u>: '.smartschool_render('user', $account, 'field_user_sms_sl_clausulering').'</li>';
			                            }
		                            }
		                            if ($traject){
		                        ...
		                        if (count($account->field_user_sms_sl2_schooljaar)){
		                            ...
		                            if (array_key_exists(LANGUAGE_NONE, $account->field_user_sms_sl2_schooljaar)){
			                            if ($account->field_user_sms_sl2_schooljaar[LANGUAGE_NONE][0]['safe_value']){
			                                $trajectTitle = '<h5>'.smartschool_render('user', $account, 'field_user_sms_sl2_schooljaar').'</h5>';
			                            }
		                            }
		                            ...
		                            if (array_key_exists(LANGUAGE_NONE, $account->field_user_sms_sl2_school)){
			                            if ($account->field_user_sms_sl2_school[LANGUAGE_NONE][0]['safe_value']){
			                                $traject .= '<li>'.smartschool_render('user', $account, 'field_user_sms_sl2_school');
			                                if ($account->field_user_sms_sl2_stad_school[LANGUAGE_NONE][0]['safe_value']){
			                                    $traject .= ' ('.smartschool_render('user', $account, 'field_user_sms_sl2_stad_school').')';
			                                }
			                                $traject .= '</li>';
			                            }
		                            }
		                            ...
		                            if (array_key_exists(LANGUAGE_NONE, $account->field_user_sms_sl2_leerjaar_en_s)){
			                            if ($account->field_user_sms_sl2_leerjaar_en_s[LANGUAGE_NONE][0]['safe_value']){
			                                $traject .= '<li>'.smartschool_render('user', $account, 'field_user_sms_sl2_leerjaar_en_s').'</li>';
			                            }
		                            }
		                            ...
		                            if (array_key_exists(LANGUAGE_NONE, $account->field_user_sms_sl2_attest)){
			                            if ($account->field_user_sms_sl2_attest[LANGUAGE_NONE][0]['safe_value']){
			                                $traject .= '<li><strong>'.smartschool_render('user', $account, 'field_user_sms_sl2_attest').'</strong> behaald</li>';
			                            }
			                            if ($traject){
			                                print '<li>'.$trajectTitle.'<ul>'.$traject.'</ul></li>';
			                            }
