...
function argus_gebruikersregistratie_form_pupil($form, &$form_state) {
	...
	$form['algemeen'] = array(
		...
		$options = argus_get_user_select_options('lid van de groep Inschrijvingsteam');
		...
		$options = db_query($query, array(':bundle' => 'klas'))->fetchAllKeyed();
...
Buitengewoon secundair onderwijs komt hiervoor niet in aanmerking.</li><li>Hoger onderwijs afgewerkt = Dit is een diploma van het een hogeschool of van een universiteit, bijvoorbeeld A1, B1, gegradueerde, licentiaat, ingenieur, doctor, master, bachelor.</li></ul>',
		...
		$form['beginsituatie']['medische gegevens'] = array(
			...
			$form['beginsituatie']['medische gegevens']['diagnose_speciali'] = array(
					...
					'#states' => array(
							'visible' => array(
									array(
											array( ':input[name="astma"]' => array('value' => 'Ja') ),
											'or',
											array( ':input[name="allergie"]' => array('value' => 'Ja') ),
											'or',
											array( ':input[name="epilepsie"]' => array('value' => 'Ja') ),
											'or',
											array( ':input[name="diabetes"]' => array('value' => 'Ja') ),
											'or',
											array( ':input[name="concentratieprobl"]' => array('value' => 'Ja') ),
											'or',
											array( ':input[name="andere_mp"]' => array('!value' => '') ),
									),
							),
					),
					'#required' => FALSE,
			...
			$form['beginsituatie']['medische gegevens']['specialist'] = array(
					...
					'#states' => array(
							'visible' => array(
									':input[name="diagnose_speciali"]' => array('value' => 'Ja'),
							),
					),
			);
			...
			$form['beginsituatie']['medische gegevens']['doktersattest_'] = array(
					...
					'#states' => array(
							'visible' => array(
									':input[name="diagnose_speciali"]' => array('visible' => TRUE),
							),
					),
			);
			$form['beginsituatie']['medische gegevens']['da_voor_hoe_lang_'] = array(
					...
					'#states' => array(
							'visible' => array(
									':input[name="doktersattest_"]' => array('value' => 'Ja'),
							),
					),
			);
			...
			$form['beginsituatie']['medische gegevens']['welke_en_hoeveel_'] = array(
					...
					'#states' => array(
							'visible' => array(
									':input[name="medicatie_"]' => array('value' => 'Ja'),
							),
					),
			);
			...
			$form['beginsituatie']['medische gegevens']['wanneer_ingeent_t'] = array(
					...
					'#states' => array(
							'visible' => array(
									':input[name="ingeent_tegen_kle"]' => array('value' => 'Ja'),
							),
					),
			);
		...
		$form['beginsituatie']['leermoeilijkheden'] = array(
			...
			$form['beginsituatie']['leermoeilijkheden']['attest_leerstoorn'] = array(
					...
					'#states' => array(
							'visible' => array(
									array(
											array( ':input[name="dyslexie"]' => array('value' => t('Ja')) ),
											'or',
											array( ':input[name="dyscalculie"]' => array('value' => t('Ja')) ),
											'or',
											array( ':input[name="adhd"]' => array('value' => t('Ja')) ),
											'or',
											array( ':input[name="ass__autisme_spec"]' => array('value' => t('Ja')) ),
											'or',
											array( ':input[name="nld__non_verbale_"]' => array('value' => t('Ja')) ),
											'or',
											array( ':input[name="andere_ls"]' => array('!value' => '') ),
									),
							),
					),
			);
			...
			$form['beginsituatie']['leermoeilijkheden']['attest_ingeleverd'] = array(
					...
					'#states' => array(
							'visible' => array(
									':input[name="attest_leerstoorn"]' => array('value' => t('Ja')),
							),
					),
			);
			...
			$form['beginsituatie']['leermoeilijkheden']['diagnose_deskundi'] = array(
					...
					'#states' => array(
							'visible' => array(
									':input[name="attest_leerstoorn"]' => array('visible' => TRUE),
							),
					),
			);
			$form['beginsituatie']['leermoeilijkheden']['deskundige'] = array(
					...
					'#states' => array(
							'visible' => array(
									':input[name="diagnose_deskundi"]' => array('value' => t('Ja')),
							),
					),
			);
			...
			$form['beginsituatie']['leermoeilijkheden']['buitengewoon_onde'] = array(
				...
				$form['beginsituatie']['leermoeilijkheden']['buso'] = array(
						...
						'#states' => array(
								'visible' => array(
										':input[name="buitengewoon_onde"]' => array('value' => t('Ja')),
								),
						),
				);
	...
	$form['praktisch'] = array(
		...
		$form['praktisch']['intern_extern_'] = array(
			...
			$form['praktisch']['internaat'] = array(
					...
					'#states' => array(
							'visible' => array(
									':input[name="intern_extern_"]' => array('value' => t('Ja')),
							),
					),
			);
		...
		$form['praktisch']['contact'] = array(
				...
				'#options' => array(
						...
						'_ande' => t('Ander kanaal'), //contact_via__ande
				),
				'#required' => FALSE,
		);
	...
	$form['readyForSubmit'] = array(
		...
		$form['readyForSubmit']['submit'] = array(
			'#type' => 'submit',
			'#validate' => array('argus_gebruikersregistratie_form_pupil_validate'),
			'#submit' => array('argus_gebruikersregistratie_form_pupil_submit'),
			'#weight' => 10,
...
function argus_gebruikersregistratie_form_pupil_submit($form, &$form_state) {
	...
	do{
		$query = 'SELECT u.uid '
				. 'FROM {users} AS u '
				. 'WHERE u.name = :account AND u.uid != :uid';
		$checkAccounts = db_query($query, array(':account' => $accountCheck, ':uid' => $uid))->rowCount();
		if ($checkAccounts){
	...
	$password = 'KTA1Hasselt_'.rand(1000,9999);
	...
	$user_data['field_user_tmp_pwco1'][LANGUAGE_NONE][0]['value'] = 'KTA1Hasselt_'.rand(1000,9999);
	$user_data['field_user_tmp_pwco2'][LANGUAGE_NONE][0]['value'] = 'KTA1Hasselt_'.rand(1000,9999);
	...
	switch ($form_state['values']['type_coaccount1']){
		case 'Moeder':
			...
			$tel = implode(' - ', array($form_state['values']['telnummer_coac1'], $form_state['values']['mobielnr_coac1']));
	...
	switch ($form_state['values']['type_coaccount2']){
		case 'Moeder':
			...
			$tel = implode(' - ', array($form_state['values']['telnummer_coac2'], $form_state['values']['mobielnr_coac2']));
			$user_data['field_user_sms_telefoon_moeder'][LANGUAGE_NONE][0]['value'] = $tel;
			break;
		case 'Vader':
	...
	$optionsContact = array( 'adver', 'clb', 'famil', 'opend', 'schoo', 'tv', 'websi', '_ande' );
