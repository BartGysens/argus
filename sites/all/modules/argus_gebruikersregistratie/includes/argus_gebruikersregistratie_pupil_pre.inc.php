...
function argus_gebruikersregistratie_form_pupil_pre($form, &$form_state) {
	...
	$form['readyForSubmit'] = array(
		...
		$form['readyForSubmit']['submit'] = array(
			'#type' => 'submit',
			'#validate' => array('argus_gebruikersregistratie_form_pupil_pre_validate'),
			'#submit' => array('argus_gebruikersregistratie_form_pupil_pre_submit'),
			'#weight' => 10,
...
function argus_gebruikersregistratie_form_pupil_pre_submit($form, &$form_state) {
	...
	do{
		$query = 'SELECT u.uid '
				. 'FROM {users} AS u '
				. 'WHERE u.name = :account AND uid != :uid';
