...
function argus_gebruikersregistratie_form_pupil_renewal($form, &$form_state) {
	...
	$form['readyForSubmit'] = array(
		...
		$form['readyForSubmit']['submit'] = array(
			'#type' => 'submit',
			'#validate' => array('argus_gebruikersregistratie_form_pupil_renewal_validate'),
			'#submit' => array('argus_gebruikersregistratie_form_pupil_renewal_submit'),
			'#weight' => 10,
...
function argus_gebruikersregistratie_form_pupil_renewal_submit($form, &$form_state) {
	...
	try {
		...
		if ($u){
			argus_report('De gebruiker "%accountname" werd succesvol heringeschreven.', array('%accountname' => $account), 'status', 'argus');
		...
		argus_report('De gebruiker "%accountname" kon niet worden heringeschreven.<br />'.$e->getMessage(), array('%accountname' => $account), 'error', 'argus');
	}
}
