 ...
 */
function argus_kerncijfers_plugin_leerlingen_leeftijd_title(){
	return t('leeftijd');
}
 ...
 */
function argus_kerncijfers_plugin_leerlingen_leeftijd_description(){
	return t('De totalen worden weergegeven op basis van de leeftijd.');
}

/**
 * Retrieve information for this plugin: type
 */
function argus_kerncijfers_plugin_leerlingen_leeftijd_type(){
	return t('staaf');
}
 ...
 */
function argus_kerncijfers_plugin_leerlingen_leeftijd_width(){
	return t('100%');
}
 ...
 */
function argus_kerncijfers_plugin_leerlingen_leeftijd_height(){
	return t('400px');
}
 ...
 */
function argus_kerncijfers_plugin_leerlingen_leeftijd_package(){
 ...
 */
function argus_kerncijfers_plugin_leerlingen_leeftijd_data(){
	$data = array(
		array( 'Leeftijd', 'Aantal leerlingen' )
	);
	
	$query = argus_kerncijfers_preprocess_query(
		  'SELECT FLOOR(DATEDIFF(CURRENT_DATE(), g.field_user_sms_geboortedatum_value) / 365) AS leeftijd, COUNT(DISTINCT u.uid) as total '
	...
	$results = db_query($query['string'], $query['options'], array())->fetchAllKeyed();
	
	foreach($results as $cl => $cntr){
		$data[] = array( (string) $cl, (int) $cntr );
	}
 ...
 */
function argus_kerncijfers_plugin_leerlingen_leeftijd_options(){
	$options = array(
		'legend' => 'none',
		'hAxis' => array(
			'textStyle' => array(
