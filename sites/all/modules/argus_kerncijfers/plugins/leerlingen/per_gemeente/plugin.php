...
function argus_kerncijfers_plugin_leerlingen_per_gemeente_data(){
	$data = array(
		array( 'Gemeente', 'Aantal leerlingen', 'Procent')
	);
	
	$query = argus_kerncijfers_preprocess_query(
		  'SELECT wp.field_user_sms_woonplaats_value AS woonplaats, COUNT(wp.entity_id) as total '
...
function argus_kerncijfers_plugin_leerlingen_per_gemeente_options($max = 1){
	...
	$cntr = db_query($query['string'], $query['options'])->fetchCol()[0];
