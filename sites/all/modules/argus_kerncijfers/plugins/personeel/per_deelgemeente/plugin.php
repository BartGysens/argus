...
function argus_kerncijfers_plugin_personeel_per_deelgemeente_data() {
	...
	$query = 'SELECT wp.field_user_sms_woonplaats_value AS woonplaats, wpd.field_user_sms_woonplaats___deel_value AS deelgemeente, CONCAT_WS({:sep},wp.field_user_sms_woonplaats_value,wpd.field_user_sms_woonplaats___deel_value) AS grouper, COUNT(DISTINCT u.uid) as total ' . 'FROM {field_data_field_user_sms_woonplaats___deel} AS wpd ' . 'INNER JOIN {field_data_field_user_sms_woonplaats} AS wp ON wpd.entity_id = wp.entity_id ' . 'INNER JOIN {users} AS u ON wp.entity_id = u.uid ' . 'INNER JOIN {users_roles} AS r ON u.uid = r.uid ' . 'WHERE u.status = 1 AND r.rid = :role ' . 'GROUP BY woonplaats,deelgemeente ' . 'ORDER BY total DESC, woonplaats ASC, deelgemeente ASC';
	$results = db_query ( $query, array (
			':sep' => ',',
			':role' => variable_get ( 'argus_engine_roles_workers' ) 
	) )->fetchAllAssoc ( 'grouper', PDO::FETCH_ASSOC );
	
	$query = 'SELECT COUNT(DISTINCT u.uid) as total ' . 'FROM {field_data_field_user_sms_woonplaats} AS wp ' . 'INNER JOIN {users} AS u ON wp.entity_id = u.uid ' . 'INNER JOIN {users_roles} AS r ON u.uid = r.uid ' . 'WHERE u.status = 1 AND r.rid = :role ';
...
function argus_kerncijfers_plugin_personeel_per_deelgemeente_options($max = 1) {
	$query = 'SELECT wp.field_user_sms_woonplaats_value AS woonplaats, wpd.field_user_sms_woonplaats___deel_value AS deelgemeente, CONCAT_WS({:sep},wp.field_user_sms_woonplaats_value,wpd.field_user_sms_woonplaats___deel_value) AS grouper, COUNT(DISTINCT u.uid) as total ' . 'FROM {field_data_field_user_sms_woonplaats___deel} AS wpd ' . 'INNER JOIN {field_data_field_user_sms_woonplaats} AS wp ON wpd.entity_id = wp.entity_id ' . 'INNER JOIN {users} AS u ON wp.entity_id = u.uid ' . 'INNER JOIN {users_roles} AS r ON u.uid = r.rid ' . 'WHERE u.status = 1 AND r.rid != :role ' . 'GROUP BY woonplaats,deelgemeente ' . 'ORDER BY total DESC, woonplaats ASC, deelgemeente ASC LIMIT 1';
	$results = db_query ( $query, array (
			':sep' => ',',
			':role' => variable_get ( 'argus_engine_roles_pupil' ) 
	) )->fetchAllAssoc ( 'grouper', PDO::FETCH_ASSOC );
	...
	$query = 'SELECT COUNT(DISTINCT u.uid) as total ' . 'FROM {field_data_field_user_sms_woonplaats} AS wp ' . 'INNER JOIN {users} AS u ON wp.entity_id = u.uid ' . 'INNER JOIN {users_roles} AS r ON u.uid = r.rid ' . 'WHERE u.status = 1 AND r.rid != :role ';
	$total = db_query ( $query, array (
			':role' => variable_get ( 'argus_engine_roles_pupil' ) 
	) )->fetchCol () [0];
