...
function argus_kerncijfers_plugin_personeel_per_gemeente_data() {
	...
	$query = 'SELECT wp.field_user_sms_woonplaats_value AS woonplaats, COUNT(DISTINCT u.uid) as total ' . 'FROM {field_data_field_user_sms_woonplaats} AS wp ' . 'INNER JOIN {users} AS u ON wp.entity_id = u.uid ' . 'INNER JOIN {users_roles} AS r ON u.uid = r.uid ' . 'WHERE u.status = 1 AND r.rid = :role ' . 'GROUP BY wp.field_user_sms_woonplaats_value ' . 'ORDER BY total DESC, woonplaats ASC';
...
function argus_kerncijfers_plugin_personeel_per_gemeente_options($max = 1) {
	$query = 'SELECT COUNT(DISTINCT u.uid) as total ' . 'FROM {field_data_field_user_sms_woonplaats} AS wp ' . 'INNER JOIN {users} AS u ON wp.entity_id = u.uid ' . 'INNER JOIN {users_roles} AS r ON u.uid = r.rid ' . 'WHERE u.status = 1 AND r.rid != :role ' . 'GROUP BY wp.field_user_sms_woonplaats_value ' . 'ORDER BY total DESC LIMIT 1';
	$results = db_query ( $query, array (
	...
	$options = array (
			...
			'sizeAxis' => array (
					'minValue' => 0,
					'maxValue' => round ( 100 * $cntr / $total, 2 ) 
			),
			'colorAxis' => array (
					'colors' => array (
							'#ffbcdc',
