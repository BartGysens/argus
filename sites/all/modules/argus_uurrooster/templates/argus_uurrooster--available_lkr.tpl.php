...
if ($role){
	...
	$allTeachers = count(db_query($query, array(':rid' => $role->rid))->fetchCol());
}
...
<h2><?php print t('Beschikbare leerkrachten'); ?></h2>
<em><?php print t('(aantal vrije leerkrachten in het overzicht, klikken op een blok voor meer details; de roosters van de leerkrachten kan je rechts opvragen)'); ?></em>
	...
	foreach ($periods['node'] as $p => $period) {
	    ...
	    for ($d = 0; $d < $days; $d++) {
	        ...
	        if ($currentPeriod){
		        ...
		        $currentTeachers = count(db_query($query, array(':pid' => key($currentPeriod['node'])))->fetchCol());
		        
		        echo '<td id="lesson_'.key($currentPeriod['node']).'"';
		        if ($currentTeachers>0 && ($allTeachers - $currentTeachers)) {
		            echo ' class="lesson pack'.key($currentPeriod['node']).' available" onclick="argus_uurrooster_getDetails('.key($currentPeriod['node']).', \'free\')">'.($allTeachers - $currentTeachers);
		        } else {
		            echo '>';
		        }
	        } else {
