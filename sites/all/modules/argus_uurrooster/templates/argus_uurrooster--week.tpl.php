...
<table class="argus_uurrooster_schedule_nav">
	<tr>
		<td id="argus_uurrooster_schedule_nav_previous"><a href="<?php print url(current_path(), array('absolute' => TRUE)).'?s='.$previousStartOfWeek; ?>">&lt; vorige week</a></td>
		<td id="argus_uurrooster_schedule_nav_today"><a href="<?php print url(current_path(), array('absolute' => TRUE)); ?>">deze week</a></td>
		<td id="argus_uurrooster_schedule_nav_next"><a href="<?php print url(current_path(), array('absolute' => TRUE)).'?s='.$nextStartOfWeek; ?>">volgende week &gt;</a></td>
	</tr>
</table>
...
foreach ($periods['node'] as $p => $period) {
    ...
    for ($d = 0; $d < $days; $d++) {
        ...
        if ($currentPeriod){
	        ...
	        $query->entityCondition('entity_type', 'node')
	            ...
	            ->propertyCondition('status', NODE_PUBLISHED)
	            ->fieldCondition('field_uurrooster_les_periode', 'target_id', $pid, '=');
	        if ($cid) {
		        ...
		        // Check substitutions
	        	$queryS = 'SELECT DISTINCT(l.field_uurrooster_perm_vervanger_target_id) AS id '
		        		. 'FROM {field_data_field_uurrooster_perm_vervanger} AS l '
	    				. 'INNER JOIN {field_data_field_uurrooster_perm_periode} AS p ON p.entity_id = l.entity_id '
						. 'WHERE l.field_uurrooster_perm_vervanger_target_id = :uid '
	    				. 'AND p.field_uurrooster_perm_periode_target_id = :pid';
		        $substitution = db_query($queryS, array(':uid' => $uid, ':pid' => $pid))->fetchCol();
		        
		        // Check supervisions
		        $queryS = 'SELECT DISTINCT(t.entity_id) AS nid, n.title AS title '
		        		. 'FROM {field_data_field_uurrooster_toez_toezichter} AS t '
		        		. 'INNER JOIN {field_data_field_uurrooster_toez_locatie} AS l ON t.entity_id = l.entity_id '
		        		. 'INNER JOIN {field_data_field_uurrooster_toez_start} AS p ON p.entity_id = l.field_uurrooster_toez_locatie_target_id '
		        		. 'INNER JOIN {field_data_field_uurrooster_toez_dag} AS d ON d.entity_id = l.entity_id '
		        		. 'INNER JOIN {node} AS n ON n.nid = t.entity_id '
		        		. 'WHERE t.field_uurrooster_toez_toezichter_target_id = :uid '
		        		. 'AND p.field_uurrooster_toez_start_target_id = :pid '
		        		. 'AND d.field_uurrooster_toez_dag_value = :did';
		        $supervision = db_query($queryS, array(':uid' => $uid, ':did' => $d+1, ':pid' => $pid))->fetchAssoc();
	        ...
	        if (count($currentLesson)>0) {
	            ...
	            $lessonSubstituted = '';
	            $substitute = '';
	            $querySubs = 'SELECT vu.field_gebruiker_target_id AS uid ' .
	            		'FROM {field_data_field_uurr_vervanging_vervmoment} AS vm ' .
	            		'INNER JOIN {field_data_field_gebruiker} AS vu ON vu.entity_id = vm.entity_id ' .
	            		'INNER JOIN {field_data_field_tijdstip} AS t ON t.entity_id = vm.entity_id ' .
	            		'WHERE vm.field_uurr_vervanging_vervmoment_target_id = :lid AND vu.bundle = :bundle ' .
	            		'AND t.bundle = :bundle AND DATE_FORMAT(t.field_tijdstip_value,\'%Y-%m-%d\') = :start';
	            $substitutions = db_query($querySubs, array(':bundle' => 'uurrooster_vervanging', ':lid' => $currentLesson['nid'], ':start' => $thisDay->format('Y-m-d')))->fetchAllAssoc('uid');
	        	...
	        	$querySubs = 'SELECT vu.field_gebruiker_target_id AS uid ' .
	        			'FROM {field_data_field_uurr_vervanging_vervmoment} AS vm ' .
	        			'INNER JOIN {field_data_field_uurrooster_perm_periode} AS p ON p.entity_id = vm.field_uurr_vervanging_vervmoment_target_id ' .
	        			'INNER JOIN {field_data_field_gebruiker} AS vu ON vu.entity_id = vm.entity_id ' .
	        			'INNER JOIN {field_data_field_tijdstip} AS t ON t.entity_id = vm.entity_id ' .
	        			'WHERE p.field_uurrooster_perm_periode_target_id = :pid AND vu.bundle = :bundle ' .
	        			'AND t.bundle = :bundle AND DATE_FORMAT(t.field_tijdstip_value,\'%Y-%m-%d\') = :start';
	        	$substitutions = db_query($querySubs, array(':bundle' => 'uurrooster_vervanging', ':pid' => $pid, ':start' => $thisDay->format('Y-m-d')))->fetchAllAssoc('uid');
	        	if (count($substitutions)>0){
	        	...
	        	$substituteNow = '';
	        	$querySubs = 'SELECT vm.field_uurr_vervanging_vervmoment_target_id ' .
	        			'FROM {field_data_field_uurr_vervanging_vervmoment} AS vm ' .
	        			'INNER JOIN {field_data_field_gebruiker} AS vu ON vu.entity_id = vm.entity_id ' .
	        			'INNER JOIN {field_data_field_uurrooster_les_periode} AS lp ON lp.entity_id = vm.field_uurr_vervanging_vervmoment_target_id ' .
	        			'INNER JOIN {field_data_field_tijdstip} AS t ON t.entity_id = vm.entity_id ' .
	        			'WHERE lp.field_uurrooster_les_periode_target_id = :pid AND vu.field_gebruiker_target_id = :uid ' .
	        			'AND t.bundle = :bundle AND DATE_FORMAT(t.field_tijdstip_value,\'%Y-%m-%d\') = :start';
	        	$substitutions = db_query($querySubs, array(':bundle' => 'uurrooster_vervanging', ':pid' => $pid, ':uid' => $uid, ':start' => $thisDay->format('Y-m-d')))->fetchCol();
	        	if (count($substitutions)>0){
	...
	//        if (!isset($substituteNow) && !($rid || $cid)){ > fix om toezichten te kunnen zien na permanenties
		if (!($rid || $cid)){
		        ...
		        $querySubs = 'SELECT DISTINCT(vm.field_uurr_vervanging_vervmoment_target_id) AS lid ' .
			        		'FROM {field_data_field_uurr_vervanging_vervmoment} AS vm ' .
			        		'INNER JOIN {field_data_field_gebruiker} AS vu ON vu.entity_id = vm.entity_id ' .
			       			'INNER JOIN {field_data_field_tijdstip} AS t ON t.entity_id = vm.entity_id ' .
		        			'INNER JOIN {field_data_field_uurrooster_les_periode} AS lp ON lp.entity_id = vm.field_uurr_vervanging_vervmoment_target_id ' .
			        		'WHERE lp.field_uurrooster_les_periode_target_id = :pid ' .
			        		'AND vu.bundle = :bundle ' .
			        		'AND vu.field_gebruiker_target_id = :uid ' .
			        		'AND t.bundle = :bundle ' .
			        		'AND DATE_FORMAT(t.field_tijdstip_value,\'%Y-%m-%d\') = :start';
		        $substitutions = db_query($querySubs, array(':bundle' => 'uurrooster_vervanging', ':uid' => $uid, ':pid' => $pid, ':start' => $thisDay->format('Y-m-d')))->fetchCol();
		        if (count($substitutions)>0){
		        ...
		        if (is_array($supervision) && isset($startOfLesson)) {
		        	...
		        	$querySubs = 'SELECT vu.field_gebruiker_target_id AS uid ' .
		        			...
		        			'INNER JOIN {field_data_field_gebruiker} AS vu ON vu.entity_id = vm.entity_id ' .
		        			'INNER JOIN {field_data_field_tijdstip} AS t ON t.entity_id = vm.entity_id ' .
		        			'WHERE vm.field_uurr_vervanging_vervmoment_target_id = :sid AND vu.bundle = :bundle ' .
		        			'AND t.bundle = :bundle AND DATE_FORMAT(t.field_tijdstip_value,\'%Y-%m-%d\') = :start';
		        	$substitutions = db_query($querySubs, array(':bundle' => 'uurrooster_vervanging', ':sid' => $supervision['nid'], ':start' => $thisDay->format('Y-m-d')))->fetchAllAssoc('uid');
		        	if (count($substitutions)>0){
		        		$needsSubstitution = ' substituted';
		        		if (key($substitutions) == 0){
		        			$needsSubstitution .= 'NotOK';
		        ...
		        $querySubs = 'SELECT n.title AS title '
			        		. 'FROM {field_data_field_uurr_vervanging_vervmoment} AS vm '
			        		. 'INNER JOIN {field_data_field_uurrooster_toez_locatie} AS l ON vm.field_uurr_vervanging_vervmoment_target_id = l.entity_id '
			        		. 'INNER JOIN {field_data_field_uurrooster_toez_start} AS p ON p.entity_id = l.field_uurrooster_toez_locatie_target_id '
			        		. 'INNER JOIN {field_data_field_uurrooster_toez_dag} AS d ON d.entity_id = l.entity_id '
			        		. 'INNER JOIN {node} AS n ON n.nid = vm.field_uurr_vervanging_vervmoment_target_id '
			        		. 'INNER JOIN {field_data_field_gebruiker} AS vu ON vu.entity_id = vm.entity_id '
			        		. 'INNER JOIN {field_data_field_tijdstip} AS t ON t.entity_id = vm.entity_id '
			        		. 'WHERE vu.bundle = :bundle AND vu.field_gebruiker_target_id = :uid '
			        		. 'AND p.field_uurrooster_toez_start_target_id = :pid '
			        		. 'AND d.field_uurrooster_toez_dag_value = :did '
			        		. 'AND t.bundle = :bundle AND DATE_FORMAT(t.field_tijdstip_value,\'%Y-%m-%d\') = :start';
		        $substitutions = db_query($querySubs, array(':bundle' => 'uurrooster_vervanging', ':did' => $d+1, ':pid' => $pid, ':uid' => $uid, ':start' => $thisDay->format('Y-m-d')))->fetchAssoc();
		        if ($substitutions){
			        $supervisions = '';
